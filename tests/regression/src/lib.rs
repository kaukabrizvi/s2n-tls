...
impl CertKeyPair {
    ...
    pub fn ecdsa() -> Self {
        CertKeyPair {
            ...
            cert: &include_bytes!("../../../tests/pems/ecdsa_p256_pkcs1_cert.pem")[..],
            key: &include_bytes!("../../../tests/pems/ecdsa_p256_pkcs1_key.pem")[..],
        }
    }
}
...
pub fn create_config(cipher_prefs: &security::Policy, keypair: CertKeyPair) -> Result<s2n_tls::config::Config, Error> {
    let builder = Builder::new();
    let builder = configure_config(builder, cipher_prefs)?;
    let builder = create_cert(builder, keypair)?;
    let config = builder.build().expect("Unable to build server config");
    Ok(config)
}
...
pub fn create_cert(mut builder: s2n_tls::config::Builder, keypair: CertKeyPair) -> Result<s2n_tls::config::Builder, Error> {
    builder
